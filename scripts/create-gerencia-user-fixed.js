// Script para crear usuario de gerencia - Versión corregida
  ...
  try {
    ...
    if (checkUser.rows.length > 0) {
      ...
      const insertUser = await client.query(
        `INSERT INTO auth_users (username, email, password_hash, role, fecha_ingreso, is_active, created_at, updated_at)
      ...
      console.log('✅ Usuario de gerencia creado con ID:', userId);
    }
    ...
    if (checkPermissions.rows.length > 0) {
      ...
      await client.query(
        ...
        VALUES ($1, true, false, true, false, false, true, false, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
        [userId]
      );
    ...
    console.error('❌ Error al crear usuario de gerencia:', error.message);
