// migrate-retired-associates.js - Script para aplicar las migraciones de asociados retirados
...
async function createRetiredAssociatesTables() {
  ...
  try {
    ...
    await query('CREATE INDEX IF NOT EXISTS idx_retired_associates_cedula ON retired_associates(cedula);');
    await query('CREATE INDEX IF NOT EXISTS idx_retired_associates_zone ON retired_associates(zona);');
    await query('CREATE INDEX IF NOT EXISTS idx_retired_associates_date ON retired_associates(retired_date);');
    ...
    await query('CREATE INDEX IF NOT EXISTS idx_retired_history_associate ON retired_associate_supply_history(retired_associate_id);');
    await query('CREATE INDEX IF NOT EXISTS idx_retired_history_date ON retired_associate_supply_history(delivered_at);');
    console.log('✅ Índices de retired_associate_supply_history creados');

    // Verificar que las tablas se crearon correctamente
    const retiredAssociatesCheck = await query('SELECT COUNT(*) as count FROM retired_associates;');
    const retiredHistoryCheck = await query('SELECT COUNT(*) as count FROM retired_associate_supply_history;');
...
module.exports = { createRetiredAssociatesTables };
